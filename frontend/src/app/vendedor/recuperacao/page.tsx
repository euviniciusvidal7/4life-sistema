...
export default function RecuperacaoPage() {
  ...
  async function carregar() {
    ...
    try {
      ...
      const resp = await fetch(`${apiUrl}/api/leads/meus?status=distribuido&tipo=recuperacao`, {
  ...
  async function processarTodos() {
    try {
      ...
      toast.success(`Confirmados: ${ok} • Falhas: ${fail}`)
      await carregar()
    } catch {
  ...
  useEffect(() => {
    ...
    // SSE: atualizar quando um lead for atribuído
    const token = localStorage.getItem('token')
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3 text-sm">
                  ...
                  <button onClick={processarTodos} className="px-3 py-1.5 bg-emerald-600 text-white rounded hover:bg-emerald-700">Confirmar Todos</button>
                </div>
              </div>
            </CardHeader>
            <CardContent>
              {loading ? (
                ...
                <p className="text-gray-500 text-sm">Sem leads de recuperação.</p>
              ) : (
